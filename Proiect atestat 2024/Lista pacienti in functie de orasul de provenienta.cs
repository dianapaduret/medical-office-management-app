...
{
    public partial class Lista_pacienti_in_functie_de_orasul_de_provenienta : Form
    {
        string qs;
        SqlConnection con;
        SqlCommand cmd;
        SqlDataAdapter da;
        DataTable dt;
        public Lista_pacienti_in_functie_de_orasul_de_provenienta()
        {
            InitializeComponent();
        ...
        private void buttonAfiseaza_Click(object sender, EventArgs e)
        {
            qs = $@"SELECT NumePacient, Adresa, Localitate, Oras, Sex, Varsta, GrupaSange, NumeDoctor FROM pacienti
                WHERE Oras='{textBoxOras.Text}' ORDER BY NumePacient";
            afisare(qs);
        }
    }
}
