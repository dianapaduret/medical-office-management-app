...
{
    public partial class Lista_pacienti_in_functie_de_specializarea_medicului : Form
    {
        string qs;
        SqlConnection con;
        SqlCommand cmd;
        SqlDataAdapter da;
        DataTable dt;
        public Lista_pacienti_in_functie_de_specializarea_medicului()
        {
            InitializeComponent();
        ...
        private void buttonAfiseaza_Click(object sender, EventArgs e)
        {
            qs = $@"SELECT NumePacient, Adresa, Localitate, Oras, Sex, Varsta, GrupaSange, NumeDoctor FROM pacienti
                WHERE NumeDoctor='{textBoxPacientSpMed.Text}' ORDER BY NumePacient";
            afisare(qs);
        }
    }
}
